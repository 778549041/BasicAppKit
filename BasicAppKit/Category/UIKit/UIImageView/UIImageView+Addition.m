...
- (void)setImageWithURLString:(NSString *)url withPlaceholder:(NSString *)imgName{
...
+ (NSArray *)getImageArray:(NSString *)filePath{
    NSFileManager *file = [NSFileManager defaultManager];
    NSString *path = [[NSBundle mainBundle] pathForResource:filePath ofType:@"bundle"];
    NSArray *arrays = [file contentsOfDirectoryAtPath:path error:nil];
    NSMutableArray *imagesArr = [NSMutableArray array];
    for (NSString *name in arrays) {
        UIImage *image = [UIImage imageNamed:[[NSString stringWithFormat:@"%@%@",filePath,@".bundle"] stringByAppendingPathComponent:name]];
        if (image) {
...
- (void)setGitPath:(NSString *)filePath Succeed:(void (^)(void))succeed{
